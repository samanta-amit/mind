...
// #endif
	...
	// preempt_disable();
	// while (!kthread_should_stop() && w->run) {
	while (w->run) {
...
terminate_worker:
	// BUG();
	w->flags = THPOOL_WORKER_terminated;
	pr_info("thpool: CPU%2d %s worker_id: %d terminated\n",
		smp_processor_id(), current->comm, thpool_worker_id(w));
	return 0;
}
...
static void memory_manager_early_init(void)
{
	...
	// thpool_buffer_map = vzalloc(size);
...
void thpool_exit(void)
{
	...
	while (terminated < NR_THPOOL_WORKERS)
	{
		msleep(1000);

		terminated = 0;
		for (i = 0; i < NR_THPOOL_WORKERS; i++) {
			terminated += thpool_worker_map[i].flags == THPOOL_WORKER_terminated ? 1 : 0;
		}
		pr_info("Waiting workers...[%d/%d]\n", terminated, NR_THPOOL_WORKERS);
	}
	kfree(thpool_buffer_map);
}
